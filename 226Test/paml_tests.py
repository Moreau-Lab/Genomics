...
def branch_site_d_worker(orthogroup, workingdir):
    ...
    template.close()
    alt_ctl.close()
    cmd = ["/Users/meganbarkdull/miniconda3/bin/codeml", "og_%s_alt.ctl" % (orthogroup)]
    subprocess.call(cmd)
    ...
    alt_ctl.close()
    nul_ctl.close()
    cmd = ["/Users/meganbarkdull/miniconda3/bin/codeml", "og_%s_nul.ctl" % ( orthogroup)]
    subprocess.call(cmd)
...
def branch_positive_worker(orthogroup, workingdir):
    ...
    cml.print_options()
    cml.run(command = "/Users/meganbarkdull/miniconda3/bin/codeml", verbose = False)
...
def branch_worker(orthogroup, workingdir):
    ...
    cml.set_options(NSsites=[0])
    cml.print_options()
    cml.run(command = "/Users/meganbarkdull/miniconda3/bin/codeml", verbose = True)
    cml = codeml.Codeml(alignment = "%s/og_cds_%s.afa" % (workingdir, orthogroup), tree = "%s/og_%s.tree" % (workingdir, orthogroup), out_file = "%s/og_%s.nul" % (workingdir, orthogroup), working_dir = "%s/og_%s_working" % (workingdir, orthogroup))
...
def pairwise_yn(orthogroup, workingdir):
    ...
    yn_results = yn.run(command = "⁨/Users⁩/meganbarkdull⁩/bin⁩/paml4.8⁩/bin⁩/yn00", verbose = True)
    return yn_results
...
def aaml_worker(param_list):
    ...
    workingdir = param_list[1]
    cml = codeml.Codeml(alignment = "%s/og_cds_%s.afa" % (workingdir, orthogroup), tree = "%s/og_%s.tree" % (workingdir, orthogroup), out_file = "%s/og_%s.alt" % (workingdir, orthogroup), working_dir = "%s/og_%s_working" % (workingdir, orthogroup))
    cml.set_options(runmode=0,fix_blength=0,seqtype=2,model=3, icode=0, clock = 0, aaDist=0, Mgene = 0, fix_kappa = 0, kappa = 2, fix_omega = 0, omega = 1, getSE = 0, RateAncestor = 0, cleandata = 0, Small_Diff = .45e-6, aaRatefile = "/Users/meganbarkdull/miniconda3/pkgs/paml-4.9-h01d97ff_5/dat/jones.dat", verbose = True)
    cml.set_options(NSsites=[0])
    cml.run(command = "/Users/meganbarkdull/miniconda3/bin/codeml", verbose = True)
...
def encprime(gene_name, cds_seq, flank_seq, outdir):
    ...
    cmd = ["/Users⁩/⁨meganbarkdull⁩/bin⁩/⁨ENCprimeUnix⁩/SeqCount", "-n", "%s/%s/%s_flanks.fa" % (outdir, gene_name, gene_name), "1"]
    subprocess.call(cmd, stdout = FNULL)
    cmd = ["/Users⁩/⁨meganbarkdull⁩/bin⁩/⁨ENCprimeUnix⁩/SeqCount", "-c", "%s/%s/%s_cds.fa" % (outdir, gene_name, gene_name), "1"]
    subprocess.call(cmd, stdout = FNULL)
    cmd = ["/Users⁩/⁨meganbarkdull⁩/bin⁩/⁨ENCprimeUnix⁩/ENCprime", "%s/%s/%s_cds.fa.codcnt" % (outdir, gene_name, gene_name), "%s/%s/%s_flanks.fa.acgtfreq" % (outdir, gene_name, gene_name), "1", "%s/%s/%s.encprime" % (outdir, gene_name, gene_name), "0", "-q"]
