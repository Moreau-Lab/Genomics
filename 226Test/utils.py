...
# from scipy.stats import stats.chi2.sf # changed because chisqprob is deprecated.
from scipy.stats import chi2 # changed because chisqprob is deprecated.
...
#    seqfile.write(">outseq\n%s\n" % (seq2[outtuple[0]:outtuple[1]]))
    ...
    seqfile.close()
    cmd = ["/Users/meganbarkdull/miniconda3/bin/fsa", "%s/fsa_files_%s/%s_%s/OG_%s_%s_%s.fa" % (out_path, seq_type, inspecies, outspecies, og_num, inspecies, outspecies)]
    FNULL = open(os.devnull, 'w')
...
def prank_align_worker(og_file, outdir, use_backbone, phylogeny_file):
    ...
    if use_backbone:
        ...
        cmd = ["/Users/meganbarkdull/miniconda3/bin/prank", "-d=%s" % og_file, "-o=%s/og_cds_%s" % (outdir, og_num), "-codon", "-F", "-t=%s/og_%s.tree" % (outdir,og_num)]
        subprocess.call(cmd)
        gblock("%s/og_cds_%s.1.fas" % (outdir, og_num))
    else:
        cmd = ["/Users/meganbarkdull/miniconda3/bin/prank", "-d=%s" % og_file, "-o=%s/og_cds_%s" % (outdir, og_num), "-codon", "-F"]
        subprocess.call(cmd)


def gblock(inalignment):
    #run gblocks on given file
    cmd = ["/Users/meganbarkdull/miniconda3/bin/gblocks", inalignment, "-t=c", "-b5=h"]
    subprocess.call(cmd)

def trimal_automated(inalignment, outalignment):
    #run trimAl on given file
    cmd = ["/Users/meganbarkdull/miniconda3/bin/trimal", "-automated1", "-in", inalignment, "-out", outalignment]
    subprocess.call(cmd)
...
#    og_num = cur_og.split("_")[2].split(".fa")[0]
    ...
    if iscoding:
        cmd = ["/Users/meganbarkdull/miniconda3/bin/fsa", "--nucprot", "%s" % form_og_file]
    else:
        cmd = ["/Users/meganbarkdull/miniconda3/bin/fsa", "%s" % form_og_file]
    FNULL = open(os.devnull, 'w')
    if iscoding:
...
def discordance_worker(param_list):
    ...
    outfile.close()
    FNULL = open(os.devnull, 'w')
    cmd = ["/Users/meganbarkdull/miniconda3/bin/FastTree", "-gamma", "-nt", "-gtr", "-nome", "-mllen", "-intree", "%s/RAxML_bestTree.og_%s.tree" % (genetreedir, og), "-log", "%s/og_%s_novel.log" % (outdir, og), "%s/og_cds_%s.afa" % (outdir, og)]
    with open("%s/og_%s_novel.ftlen" % (outdir, og), 'w') as outfile:
        subprocess.call(cmd, stdout = outfile, stderr = FNULL)
    outfile.close() 

    cmd = ["/Users/meganbarkdull/miniconda3/bin/FastTree", "-gamma", "-nt", "-gtr", "-nome", "-mllen", "-intree", "%s/og_%s_consensus.tree" % (outdir, og), "-log", "%s/og_%s_consensus.log" % (outdir, og), "%s/og_cds_%s.afa" % (outdir, og)]
